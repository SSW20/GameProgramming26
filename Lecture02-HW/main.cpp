...
void UpdateVerticies(Vertex vertices[], int length, Direction dir);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
        ...
        {
            g_gamecontext.PosX = speedRate;
            g_gamecontext.PosY = 0;
            g_direction = Left;
        }
        ...
        {
            g_gamecontext.PosY = speedRate;
            g_gamecontext.PosX = 0;
            g_direction = UP;
        }
...
void UpdateVerticies(Vertex vertices[], int length, Direction dir)
{
    ...
    if (dir == UP)
    {
        ...
        {
            vertices[i].y += g_gamecontext.PosY * !keyReleased;
        }
    }
    else if (dir == Left)
    {
        for (int i = 0; i < length; ++i)
        {
            vertices[i].x -= g_gamecontext.PosX * !keyReleased;
        }
    }
    else if (dir == Right)
    {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hWnd = CreateWindowW(L"DX11GameLoopClass", L"과제: 움직이는 육망성 만들기",
        WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 600, 600, nullptr, nullptr, hInstance, nullptr);
    if (!hWnd) return -1;
    ...
    // 4. 정점 버퍼 생성 (삼각형 데이터)
    Vertex vertices[] = {
       {0.0f,  0.577f, 0.0f,  1.0f, 1.0f, 1.0f, 1.0f},
        {0.5f, -0.289f, 0.0f,  1.0f, 1.0f, 1.0f, 1.0f},
        {-0.5f, -0.289f, 0.0f,  1.0f, 1.0f, 1.0f, 1.0f},
    ...
    // --- [5. 정석 게임 루프] ---
    // Peek 메세지는 없으면 넘어감 , 게임은 인터렉티브이기 때문에 Polling 방식인 PeekMsg를 자주 씀.
    MSG msg = { 0 };
    while (WM_QUIT != msg.message) {
        ...
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            UpdateVerticies(vertices, vertexCount, g_direction);
            g_pd3dDevice->CreateBuffer(&bd, &initData, &pVBuffer);
